...
class MainWindow(CTk):
    ...
    def toggle_show_menu(self):
        if self.is_show_menu:
            ...
            self.show_menu()
            # Удаляем элементы меню при закрытии
            if self.label:
                self.label.destroy()
                self.label = None
            if self.entry:
            ...
            self.clear_button = CTkButton(self.menu_frame, text="Очистити поле вводу", command=self.clear_message_entry)
            self.clear_button.pack(pady=5, padx=10)

            self.theme_button = CTkButton(self.menu_frame, text="Змінити тему", command=self.toggle_theme)
            self.theme_button.pack(pady=5, padx=10)

            self.exit_button = CTkButton(self.menu_frame, text="Вийти", command=self.destroy)
